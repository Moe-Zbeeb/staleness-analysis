# Qwen2.5-Math-1.5B GRPO, staleness cap 8

Fresh 1,000-update run from the original pinned base model, seed 42, matching the completed cap-4 setup: 2 trainer GPUs plus 1 inference GPU, batch size 64, group size 8, and 3,072 completion tokens. The only changed training configuration is the inclusive staleness cap (8); run identities are separate.
