...
env CUDA_VISIBLE_DEVICES="$PRIME_VISIBLE_DEVICES" uv run --no-sync rl @ "$EXPERIMENT_ROOT/config/main.toml" @ "$EXPERIMENT_ROOT/config/smoke.toml" --model.name "$RUNTIME_MODEL" --run.name "qwen25-3b-grpo-smoke-$SLURM_JOB_ID-attempt-${SLURM_RESTART_COUNT:-0}" --orchestrator.env-vars "$ORCHESTRATOR_ENV"
SMOKE_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo/qwen25-3b-grpo-smoke-$SLURM_JOB_ID-attempt-${SLURM_RESTART_COUNT:-0}"
test -s "$SMOKE_DIR/checkpoints/step_2/trainer/.metadata"
test -s "$SMOKE_DIR/checkpoints/step_2/orchestrator/progress.pt"
SMOKE_LINK=/mnt/xfs/home/mohamadzbib/projects/models/qwen25-3b-grpo-smoke-$SLURM_JOB_ID
