...
The requested first run is prepared in [configs/exact256-80gb-seed42.json](configs/exact256-80gb-seed42.json): exact lag 256, the 80 GB profile, seed 42 and 1,000 total optimizer updates. This means 256 on-policy bootstrap updates followed by 744 exact-256 updates; update 257 first consumes a queued cohort generated by `theta_0` while training `theta_256`. No training job is submitted by preparing this configuration.

See the [run preparation record](docs/runs/exact256.md) for queue capacity, checkpoint interpretation, cluster paths and launch prerequisites. The scientific recipe and imported libraries are unchanged.
