...
The requested evaluation protocol now saves and retains checkpoints every 100 optimizer updates on NFS, plus the final checkpoint, with no intermediate evaluation. Evaluation of saved milestones is a separate later workflow. The tables and proposed protocol below reflect this choice; an offline evaluator is not implemented by this update.
...
Configuration resolution confirms `orchestrator.eval = null`, now explicitly set by the study. The requested protocol performs no intermediate evaluation and retains 100-update milestones for later evaluation. The custom controller runs its finite-cohort loop rather than the upstream evaluation loop. A separate frozen held-out evaluator for saved checkpoints is still needed before making quality claims.
...
| Checkpoint retention | Save every 100 updates and at completion to the configured output directory, on NFS by default; retain every 100th and last four | Preserves milestone and final states for later evaluation without evaluation work during training |
...
| Current quality | No intermediate evaluation; 100-update milestones retained | Separately evaluate saved milestone weights, initialization and final checkpoint using frozen questions/grader/decoding |
...
2. **Prepare measurement for the saved checkpoints.** Implement a separate offline evaluation workflow and the missing training diagnostics; verify generation/consumption/checkpoint clocks. After training, load or export each retained milestone and evaluate with frozen prompts, grader and decoding settings. Do not insert intermediate evaluation into the training loop. Evaluation uses its own RNG and does not consume the training prompt stream.
