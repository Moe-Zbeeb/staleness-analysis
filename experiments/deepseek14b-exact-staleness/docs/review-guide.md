...
The current package passed all 124 tests locally and in CPU-only cluster job `2144955`. The suite covers scheduling across multiple k values and horizons, queue recovery, duplicate/version rejection, official packing and transport serialization, GRPO gradients and masking, grading timeouts, data contracts, tokenizer parity, source snapshots, checkpoint integrity and configuration resolution. Additional checks cover globally weighted paper diagnostics, raw token capture, XFS mirroring and offline evaluation provenance. Runboard checks cover file/HTTP delivery, metric clocks, outage spooling, observer shutdown and failure isolation. Job `2144955` also verified three exact rows from the live backend, including all 260 diagnostic scalars; see the [integration guide](runboard.md).
