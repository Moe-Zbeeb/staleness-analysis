...
| [evaluation/](../src/deepseek_study/evaluation/) | Official `AppState` DCP model tensors; `InferenceConfig`, `setup_vllm_env`, inference environment defaults and FP32-head worker extension | A separate pool exports completed learner checkpoints and evaluates fixed benchmark shards through vLLM; no optimizer/history loading or training-loop changes | No |
...
The custom controller does not run the stock evaluation loop. Adding evaluation settings to an upstream config alone does not launch evaluation. The separately deployed [checkpoint evaluation pool](evaluation.md) discovers completed study checkpoints, exports model weights on CPU and submits independent A100 40GB jobs. It binds its own benchmark/source/runtime identities and uses the corrected benchmark v3.2 grader; training reward remains unchanged.
...
Runboard does not change this boundary. Evaluation remains disabled inside the training controller, and checkpoint files stay on NFS. The independent evaluator publishes complete-checkpoint JSON metrics, without automatic live Runboard import. Runboard's separate observer process is excluded from the launcher's training-failure checks and receives final status after training-service shutdown. See the [Runboard guide](runboard.md) for clocks, delivery limits and configuration.
...
Deploy the evaluator in its own immutable checkout/runtime. Adding its source files changes a training source fingerprint; do not overlay this release onto a running trainer or its resume source. Evaluation protocols hash the evaluator implementation and key upstream inference files. Changing those files requires a newly validated protocol or using the original release. The [evaluator validation record](evaluation.md#validation-and-remaining-acceptance) distinguishes completed tests from the remaining real trained-checkpoint export check.
