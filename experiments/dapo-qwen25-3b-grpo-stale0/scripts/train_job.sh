set -euo pipefail
EXPERIMENT_ROOT=/mnt/nfs/home/mohamadzbib/projects/rl-infra/experiments/dapo-qwen25-3b-grpo-stale0
RL_INFRA=/mnt/nfs/home/mohamadzbib/projects/rl-infra
RUN_NAME=qwen25-3b-grpo-seed42-stale0
ATTEMPT_ID="job-$SLURM_JOB_ID-restart-${SLURM_RESTART_COUNT:-0}"
RUN_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-stale0/$RUN_NAME"
OBSERVER_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-stale0-observer/$RUN_NAME"
RECOVERY_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-stale0-recovery/$RUN_NAME"
...
MODEL_LINK=/mnt/xfs/home/mohamadzbib/projects/models/qwen25-3b-grpo-seed42-stale0
...
verify_credentials() (
    ...
    export OPIK_PROJECT_NAME=qwen25-3b-grpo-stale0
)
...
if test "$TRAINING_FINISHED" = false; then
    ...
    run_guarded uv run --no-sync torchrun --standalone --nproc-per-node=7 "$EXPERIMENT_ROOT/scripts/health_probe.py"
