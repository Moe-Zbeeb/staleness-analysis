...
env CUDA_VISIBLE_DEVICES="$PRIME_VISIBLE_DEVICES" uv run --no-sync rl @ "$EXPERIMENT_ROOT/config/main.toml" @ "$EXPERIMENT_ROOT/config/smoke.toml" --model.name "$RUNTIME_MODEL" --run.name "qwen25-math7b-grpo-smoke-$SLURM_JOB_ID-attempt-${SLURM_RESTART_COUNT:-0}" --orchestrator.env-vars "$ORCHESTRATOR_ENV"
