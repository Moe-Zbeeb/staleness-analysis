set -euo pipefail
EXPERIMENT_ROOT=/mnt/nfs/home/mohamadzbib/projects/rl-infra/experiments/dapo-qwen25-3b-grpo-stale4-5gpu
RL_INFRA=/mnt/nfs/home/mohamadzbib/projects/rl-infra
RUN_NAME=qwen25-3b-grpo-seed42-stale4-5gpu
ATTEMPT_ID="job-$SLURM_JOB_ID-restart-${SLURM_RESTART_COUNT:-0}"
RUN_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-stale4-5gpu/$RUN_NAME"
OBSERVER_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-stale4-5gpu-observer/$RUN_NAME"
RECOVERY_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-stale4-5gpu-recovery/$RUN_NAME"
...
MODEL_LINK=/mnt/xfs/home/mohamadzbib/projects/models/qwen25-3b-grpo-seed42-stale4-5gpu
...
verify_credentials() (
    ...
    export OPIK_PROJECT_NAME=qwen25-3b-grpo-stale4-5gpu
)
...
if test "$TRAINING_FINISHED" = false; then
    ...
    run_guarded uv run --no-sync torchrun --standalone --nproc-per-node=5 "$EXPERIMENT_ROOT/scripts/health_probe.py"
