...
def expected_devices(allocated, configured_files, inventory):
    require(len(allocated) == GPU_COUNT and len(set(allocated)) == GPU_COUNT, "Expected exactly three distinct Slurm GPU IDs")
    result = []
...
def numeric_selectors(expected, inventory):
    ...
    require(len(selected) == GPU_COUNT and len(set(selected)) == GPU_COUNT, "CUDA selection must contain three unique GPUs")
    return selected
...
def prepare(args):
    ...
    result = {
        ...
        "prime_visible_devices": ",".join(map(str, selected[2:] + selected[:2])),
    }
