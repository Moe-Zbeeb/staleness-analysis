...
SHARDS = [f'trainer/__{rank}_0.distcp' for rank in range(2)]
PAIRED_FILES = ['trainer/.metadata', 'orchestrator/progress.pt', *SHARDS]
OVERLAY = {'deployment': {'gpus_per_node': 3, 'num_train_gpus': 2, 'num_infer_gpus': 1}, 'inference': {'vllm': {'tensor_parallel_size': 1}}, 'orchestrator': {'ckpt': {'wait_for_weights_timeout': 7200}}}
...
class Recovery:
    def __init__(self, recovery_root, experiment_root):
        ...
        require(self.cap == 6, 'Recovery requires cap 6')
    ...
    def static(self):
        ...
        if package_path.exists():
            ...
            required_package = {'guard.py', 'health_probe.py', 'overlay.toml', 'spec.json', 'train-stale6.sh', 'validate_plan.py'}
        ...
        require(digest(self.overlay) == self.spec['overlay_sha256'], 'Recovery overlay changed')
        require(tomllib.loads(self.overlay.read_text()) == OVERLAY, 'Recovery changes settings beyond the declared three-GPU topology and startup timeout')
        ...
        require(cfg['deployment'] == {'type': 'single_node', 'gpus_per_node': 4, 'num_train_gpus': 2, 'num_infer_gpus': 2}, 'Unexpected original topology')
        require(cfg['inference']['vllm']['tensor_parallel_size'] == 2 and cfg['inference']['vllm']['data_parallel_size'] == 1, 'Unexpected original inference topology')
        require(cfg['orchestrator']['max_off_policy_steps'] == self.cap, 'Original cap mismatch')
        require(self.item['resume_step'] == 775, 'Initial recovery checkpoint changed')
        ...
        require(all(isinstance(sizes.get(name), int) and sizes[name] > 0 for name in SHARDS), 'Initial two-shard evidence missing')
        return cfg

    def checkpoint(self, step):
        checkpoint = self.run / f'checkpoints/step_{step}'
        require(all((checkpoint / name).is_file() and (checkpoint / name).stat().st_size > 0 for name in PAIRED_FILES), f'Incomplete paired two-trainer checkpoint: {step}')
        require({str(path.relative_to(checkpoint)) for path in (checkpoint / 'trainer').glob('*.distcp')} == set(SHARDS), 'Checkpoint trainer world size is not two')
...
def main():
    ...
    if args.action == 'gate':
        ...
        require(len(ids) == 3 and len(set(ids)) == 3 and all(ids), 'Allocation must expose exactly three distinct GPU identifiers')
        print(','.join(ids[2:] + ids[:2]))
        return
