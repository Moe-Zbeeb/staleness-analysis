set -euo pipefail
RECOVERY_ROOT=/mnt/nfs/home/mohamadzbib/projects/rl-infra/recoveries/7b-six-gpu-20260919
...
if test "$TRAINING_FINISHED" = false; then
    ...
    GPU_MAPPING="$GPU_RECOVERY_ROOT/validation/gpu-mapping-$ATTEMPT_ID.json"
    ...
    run_guarded timeout -k 15 240 uv run --no-sync torchrun --standalone --nproc-per-node=6 "$RECOVERY_ROOT/health_probe.py"
