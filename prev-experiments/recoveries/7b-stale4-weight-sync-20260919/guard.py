...
class Recovery:
    def __init__(self, recovery_root, experiment_root):
        ...
        self.overlay = self.root / 'overlay.toml'
        self.proof = {}
    ...
    def static(self):
        ...
        if package_path.exists():
            ...
            required_package = {'runtime_guard.py', 'runtime-provenance.json', 'weight_sync_recovery.py', 'prime-rl-source-original.json', 'prime-rl-source-manifest.json', 'upstream/clients.py', 'guard.py', 'health_probe.py', 'gpu_devices.py', 'gres-reviewed.conf', 'overlay.toml', 'spec.json', 'train-stale4.sh', 'validate_job.sh', 'validate_plan.py'}
        ...
        runtime = verify_source(self.root)
        required_runtime = {'base_prime_commit': runtime['base_prime_commit'], 'provenance_file': 'runtime-provenance.json', 'source_root': 'prime-rl-source', 'changed_files': ['src/prime_rl/orchestrator/clients.py'], 'helper': 'weight_sync_recovery.py', 'admin_read_timeout_seconds': 3600, 'update_attempts': 1, 'resume_only_after_confirmed_update': True}
        require(self.spec['runtime_recovery'] == required_runtime, 'Runtime recovery declaration changed')
        require(self.spec['prelaunch_gpu_gate'] == {'require_no_compute_processes': True, 'minimum_free_fraction': 0.85, 'minimum_total_memory_gib': 75}, 'Prelaunch allocated-GPU gate changed')
        ...
        require(digest(self.overlay) == self.spec['overlay_sha256'], 'Recovery startup overlay changed')
        require(tomllib.loads(self.overlay.read_text()) == OVERLAY, 'Recovery changes settings beyond the declared startup wait timeout')
        require(self.spec['startup_weight_wait_timeout_seconds'] == 7200, 'Declared startup timeout changed')
    ...
    def capture(self, step):
        ...
        record = {'created_at': datetime.datetime.now(datetime.timezone.utc).isoformat(), 'cap': self.cap, 'run_dir': str(self.run), 'experiment_root': str(self.experiment), 'resume_step': step, 'job_id': job, 'restart_count': int(restart), 'spec_sha256': digest(self.spec_path), 'guard_sha256': digest(Path(__file__).resolve()), 'original_guard_sha256': self.item['guard_sha256'], 'source_sha256': self.item['source_sha256'], 'data_sha256': self.item['data_sha256'], 'overlay_sha256': self.spec['overlay_sha256'], 'runtime_provenance_sha256': digest(self.root / 'runtime-provenance.json'), 'runtime_manifest_sha256': digest(self.root / 'prime-rl-source-manifest.json'), 'checkpoint_files': before, 'evidence_hashes': hashes}
    ...
    def audit(self, run, steps):
        ...
        argv_line = "sys.argv = ['rl', '@', str(root / 'config/main.toml')]"
        checkpoint_line = 'require((run / f"checkpoints/step_{resume[\'step\']}/trainer/.metadata").is_file(), \'Resume checkpoint missing\')'
        require(source.count(argv_line) == 1 and source.count(checkpoint_line) == 1, 'Original runtime assertion structure changed')
        adapted = source.replace(argv_line, argv_line + " + ['@', str(recovery_overlay)]")
        adapted = adapted.replace(checkpoint_line, "recovery_resume_evidence(root, run, resume['step'], steps)")
        namespace = dict(self.original.__dict__)
        namespace.update(recovery_overlay=self.overlay, recovery_resume_evidence=self.resume_evidence)
        ...
        report.update(recovery_spec_sha256=digest(self.spec_path), recovery_guard_sha256=digest(Path(__file__).resolve()), recovery_overlay_sha256=self.spec['overlay_sha256'], original_guard_sha256=self.item['guard_sha256'], training_manifest_sha256=self.item['data_sha256'], initial_resume_step=self.item['resume_step'], startup_weight_wait_timeout_seconds=7200, execution_segment={'trainer_gpus': 4, 'inference_gpus': 1, 'inference_tensor_parallel_size': 1, 'inference_data_parallel_size': 1}, supplemental_resume_evidence=list(self.proof.values()), frozen_experiment_files_modified=False, runtime_provenance=read(self.root / 'runtime-provenance.json'), runtime_provenance_sha256=digest(self.root / 'runtime-provenance.json'), shared_prime_repository_modified=False)
        return report
