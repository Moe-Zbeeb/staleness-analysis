...
class RecoveryEvidenceTests(unittest.TestCase):
    def setUp(self):
        ...
        (root / 'prime-rl-source-manifest.json').write_text('{}')
    ...
    def test_capture_from_another_runtime_patch_is_rejected(self):
        result = self.capture()
        (self.recovery.root / 'runtime-provenance.json').write_text('{"changed":true}')
        with self.assertRaisesRegex(ValueError, 'Capture provenance mismatch: runtime_provenance_sha256'):
            self.recovery.validate_capture(Path(result['capture_path']), 225)
