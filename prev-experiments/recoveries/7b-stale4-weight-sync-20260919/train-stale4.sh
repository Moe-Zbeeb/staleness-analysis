...
RUN_NAME=qwen25-math7b-grpo-seed42-stale4-5gpu
ATTEMPT_ID="job-$SLURM_JOB_ID-restart-${SLURM_RESTART_COUNT:-0}"
RUN_DIR="$RL_INFRA/outputs/dapo-qwen25-math7b-grpo-stale4-5gpu/$RUN_NAME"
OBSERVER_DIR="$RL_INFRA/outputs/dapo-qwen25-math7b-grpo-stale4-5gpu-observer/$RUN_NAME"
RECOVERY_DIR="$RL_INFRA/outputs/dapo-qwen25-math7b-grpo-stale4-5gpu-recovery/$RUN_NAME"
...
MODEL_LINK=/mnt/xfs/home/mohamadzbib/projects/models/qwen25-math7b-grpo-seed42-stale4-5gpu
...
mkdir -p "$EXPERIMENT_ROOT/validation" "$RECOVERY_ROOT/validation"
exec 9>"$EXPERIMENT_ROOT/validation/run.lock"
if ! flock -n 9; then
    printf 'Existing run lock is held; refusing duplicate training\n' >&2
    exit 1
fi
printf '%s\n' "$SLURM_JOB_ID" > "$EXPERIMENT_ROOT/validation/active-job.txt"
printf 'Recovery startup job=%s restart=%s node=%s\n' "$SLURM_JOB_ID" "${SLURM_RESTART_COUNT:-0}" "$(hostname)"
source "$RL_INFRA/env.sh" prime-rl
RUNTIME_SOURCE="$RECOVERY_ROOT/prime-rl-source"
export PYTHONPATH="$RECOVERY_ROOT:$RUNTIME_SOURCE/src:$RUNTIME_SOURCE/packages/prime-rl-configs/src:$EXPERIMENT_ROOT/python"
uv run --no-sync python "$RECOVERY_ROOT/runtime_guard.py" --upstream-root /mnt/nfs/home/mohamadzbib/projects/rl-infra/repos/prime-rl --output "$RECOVERY_ROOT/validation/runtime-$ATTEMPT_ID.json"
uv run --no-sync python "$RECOVERY_ROOT/guard.py" gate --experiment-root "$EXPERIMENT_ROOT"
...
verify_credentials() (
    ...
    export OPIK_PROJECT_NAME=qwen25-math7b-grpo-stale4-5gpu
)
...
if test "$TRAINING_FINISHED" = false; then
    ...
    GPU_MAPPING="$RECOVERY_ROOT/validation/gpu-mapping-$ATTEMPT_ID.json"
    export CUDA_DEVICE_ORDER=PCI_BUS_ID
    run_guarded uv run --no-sync python "$RECOVERY_ROOT/gpu_devices.py" prepare --gres-config /usr/local/etc/gres.conf --expected-gres-sha256 1a5c2624ff52be8400be632e1aed6f61b75399446b91d9eb249d19ac38307732 --output "$GPU_MAPPING"
    ...
    run_guarded timeout --signal=TERM --kill-after=15s 240s uv run --no-sync torchrun --standalone --nproc-per-node=5 "$RECOVERY_ROOT/health_probe.py" --mapping "$GPU_MAPPING"
...
ARGS=(uv run --no-sync rl @ "$EXPERIMENT_ROOT/config/main.toml" @ "$RECOVERY_ROOT/overlay.toml" --model.name "$RUNTIME_MODEL" --orchestrator.env-vars "$ORCHESTRATOR_ENV")
PRIME_VISIBLE_DEVICES=$(uv run --no-sync python "$RECOVERY_ROOT/guard.py" devices --experiment-root "$EXPERIMENT_ROOT")
printf 'CUDA_VISIBLE_DEVICES=%s\n' "$CUDA_VISIBLE_DEVICES"
printf 'SLURM_JOB_GPUS=%s\n' "${SLURM_JOB_GPUS:-unset}"
printf 'PRIME_VISIBLE_DEVICES=%s\n' "$PRIME_VISIBLE_DEVICES"
if test "$TRAINING_FINISHED" = false && test -s "$RUN_DIR/checkpoints/step_1000/trainer/.metadata" && test -s "$RUN_DIR/checkpoints/step_1000/orchestrator/progress.pt"; then
    printf 'Final paired checkpoint exists; require completion audit before further training\n' >&2
    exit 1
fi
RESUME_STEP=
if test "$TRAINING_FINISHED" = false && test -d "$RUN_DIR/checkpoints"; then
    ...
    for CHECKPOINT in "$RUN_DIR"/checkpoints/step_*; do
        ...
        if test -s "$CHECKPOINT/trainer/.metadata" && test -s "$CHECKPOINT/orchestrator/progress.pt" && test "$STEP" -lt 1000; then
            ...
            printf 'Preserving incomplete checkpoint %s in %s\n' "$CHECKPOINT" "$CHECKPOINT_RECOVERY"
            if test -s "$CHECKPOINT/orchestrator/progress.pt"; then
                sha256sum "$CHECKPOINT/orchestrator/progress.pt" > "$CHECKPOINT_RECOVERY/step-$STEP-orchestrator.sha256"
            fi
...
if test "$TRAINING_FINISHED" = true; then
    ...
    uv run --no-sync python "$RECOVERY_ROOT/guard.py" capture --experiment-root "$EXPERIMENT_ROOT" --run-dir "$RUN_DIR" --resume-step "$RESUME_STEP"
    ARGS+=(--resume.step "$RESUME_STEP")
else
    printf 'No paired checkpoint is available; refusing a fresh restart\n' >&2
    exit 1
fi
verify_credentials
unset OPIK_API_KEY OPIK_URL_OVERRIDE OPIK_WORKSPACE OPIK_PROJECT_NAME
...
if test "$STATUS" -eq 0; then
    test -s "$RUN_DIR/checkpoints/step_1000/trainer/.metadata"
    test -s "$RUN_DIR/checkpoints/step_1000/orchestrator/progress.pt"
    uv run --no-sync python "$RECOVERY_ROOT/guard.py" audit --experiment-root "$EXPERIMENT_ROOT" --run-dir "$RUN_DIR" --steps 1000
