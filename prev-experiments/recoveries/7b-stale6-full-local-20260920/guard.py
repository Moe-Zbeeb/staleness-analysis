...
class Recovery:
    def __init__(self, recovery_root, experiment_root):
        ...
        require(self.cap in [6, 8], 'Unsupported recovery cap')
    ...
    def static(self):
        ...
        if package_path.exists():
            ...
            required_package = {'guard.py', 'health_probe.py', 'overlay.toml', 'spec.json', 'train-stale6.sh', 'gpu_devices.py', 'validate_plan.py'}
        ...
        require(tomllib.loads(self.overlay.read_text()) == OVERLAY, 'Recovery changes settings beyond the declared startup wait')
        ...
        require(cfg['deployment'] == {'type': 'single_node', 'gpus_per_node': 8, 'num_train_gpus': 4, 'num_infer_gpus': 4}, 'Unexpected original topology')
        require(cfg['inference']['vllm']['tensor_parallel_size'] == 1 and cfg['inference']['vllm']['data_parallel_size'] == 4, 'Unexpected original inference topology')
        require(cfg['orchestrator']['max_off_policy_steps'] == self.cap, 'Original cap mismatch')
        require(self.item['resume_step'] == {6: 125, 8: 100}[self.cap], 'Initial recovery checkpoint changed')
...
def main():
    ...
    if args.action == 'gate':
        ...
        require(len(ids) == 8 and len(set(ids)) == 8 and all(ids), 'Allocation must expose exactly eight distinct GPU identifiers')
