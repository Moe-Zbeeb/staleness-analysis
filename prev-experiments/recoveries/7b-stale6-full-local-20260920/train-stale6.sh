...
RECOVERY_ROOT=/mnt/nfs/home/mohamadzbib/projects/rl-infra/recoveries/7b-stale6-full-local-20260920
...
run_guarded python3 "$RECOVERY_ROOT/host_memory.py"
...
if test "$TRAINING_FINISHED" = false; then
    ...
    run_guarded timeout -k 15 240 uv run --no-sync torchrun --standalone --nproc-per-node=8 "$RECOVERY_ROOT/health_probe.py" --mapping "$GPU_MAPPING"
...
printf 'Comet disabled for this execution; persistent metrics: %s\n' "$METRICS_FILE"
TRAIN_PID=
wait_for_exit() {
    PID=$1
    LIMIT=$2
