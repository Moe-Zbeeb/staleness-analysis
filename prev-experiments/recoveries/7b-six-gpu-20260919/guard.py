...
OVERLAY = {'deployment': {'gpus_per_node': 6, 'num_train_gpus': 4, 'num_infer_gpus': 2}, 'inference': {'vllm': {'data_parallel_size': 2}}}
...
class Recovery:
    ...
    def static(self):
        ...
        if package_path.exists():
            ...
            required_package = {'guard.py', 'health_probe.py', 'overlay.toml', 'spec.json', 'train-stale6.sh', 'train-stale8.sh'}
        ...
        require(tomllib.loads(self.overlay.read_text()) == OVERLAY, 'Recovery changes settings beyond the declared six-GPU topology')
...
def main():
    ...
    if args.action == 'gate':
        ...
        require(len(ids) == 6 and len(set(ids)) == 6 and all(ids), 'Allocation must expose exactly six distinct GPU identifiers')
