...
RECOVERY_ROOT=/mnt/nfs/home/mohamadzbib/projects/rl-infra/recoveries/7b-stale8-native-cvd-20260920
EXPERIMENT_ROOT=/mnt/nfs/home/mohamadzbib/projects/rl-infra/experiments/dapo-qwen25-math7b-grpo-stale8-8h100
RL_INFRA=/mnt/nfs/home/mohamadzbib/projects/rl-infra
RUN_NAME=qwen25-math7b-grpo-seed42-stale8-8h100
ATTEMPT_ID="job-$SLURM_JOB_ID-restart-${SLURM_RESTART_COUNT:-0}"
RUN_DIR="$RL_INFRA/outputs/dapo-qwen25-math7b-grpo-stale8-8h100/$RUN_NAME"
OBSERVER_DIR="$RL_INFRA/outputs/dapo-qwen25-math7b-grpo-stale8-8h100-observer/$RUN_NAME"
RECOVERY_DIR="$RL_INFRA/outputs/dapo-qwen25-math7b-grpo-stale8-8h100-recovery/$RUN_NAME"
...
MODEL_LINK=/mnt/xfs/home/mohamadzbib/projects/models/qwen25-math7b-grpo-seed42-stale8-8h100
...
run_guarded uv run --no-sync python "$RECOVERY_ROOT/host_memory.py"
run_guarded uv run --no-sync python "$RECOVERY_ROOT/validate_plan.py"
...
verify_credentials() (
    ...
    export OPIK_PROJECT_NAME=qwen25-math7b-grpo-stale8-8h100
)
...
if test "$TRAINING_FINISHED" = false; then
    ...
    test "$CUDA_VISIBLE_DEVICES" = "$RECOVERY_SLURM_CUDA_VISIBLE_DEVICES"
    run_guarded timeout -k 15 240 uv run --no-sync torchrun --standalone --nproc-per-node=5 "$RECOVERY_ROOT/health_probe.py" --mapping "$GPU_MAPPING"
