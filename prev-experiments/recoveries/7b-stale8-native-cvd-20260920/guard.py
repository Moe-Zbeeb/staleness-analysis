...
OVERLAY = {'deployment': {'gpus_per_node': 5, 'num_train_gpus': 4, 'num_infer_gpus': 1}, 'inference': {'vllm': {'data_parallel_size': 1}}}
...
class Recovery:
    ...
    def static(self):
        ...
        if package_path.exists():
            ...
            required_package = {'guard.py', 'health_probe.py', 'overlay.toml', 'spec.json', 'train-stale8.sh', 'gpu_devices.py', 'validate_plan.py', 'host_memory.py'}
        ...
        require(tomllib.loads(self.overlay.read_text()) == OVERLAY, 'Recovery changes settings beyond the declared five-GPU topology')
...
def main():
    ...
    if args.action == 'gate':
        ...
        require(len(ids) == 5 and len(set(ids)) == 5 and all(ids), 'Allocation must expose exactly five distinct GPU identifiers')
