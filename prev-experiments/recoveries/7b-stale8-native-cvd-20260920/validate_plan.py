...
for cap,item in spec['caps'].items():
 ...
 assert current.deployment.gpus_per_node==5 and current.deployment.num_train_gpus==4 and current.deployment.num_infer_gpus==1
 assert current.inference.vllm.tensor_parallel_size==1 and current.inference.vllm.data_parallel_size==1
 ...
 env=dict(os.environ,CUDA_VISIBLE_DEVICES='0,2,3,4,6')
 ...
 assert mapped=='6,0,2,3,4',mapped
