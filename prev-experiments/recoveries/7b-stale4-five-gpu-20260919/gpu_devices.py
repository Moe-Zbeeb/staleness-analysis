...
def expected_devices(allocated, configured_files, inventory):
    require(len(allocated) == GPU_COUNT and len(set(allocated)) == GPU_COUNT, "Expected exactly five distinct Slurm GPU IDs")
    result = []
...
def numeric_selectors(expected, inventory):
    ...
    require(len(selected) == GPU_COUNT and len(set(selected)) == GPU_COUNT, "CUDA selection must contain five unique GPUs")
    return selected
