...
MODEL_IDS = ["qwen25-3b-base", *[f"qwen25-3b-staleness-{cap}" for cap in (2, 4, 6, 8)]]
PROFILES = ["greedy-native", "sampled-native"]


def launch_catalogs():
    models, benchmarks, profiles = load_catalogs(EVALUATION)
    return models, benchmarks, read_json(PACKAGE / "profiles.json")
...
def verify_package():
    ...
    required.update(str(item.relative_to(ROOT)) for item in PACKAGE.iterdir() if item.suffix in (".py", ".sh") or item.name == "profiles.json")
...
def load_plan(path):
    plan = read_json(path)
    expected = make_plan(*launch_catalogs(), selected_models=MODEL_IDS, selected_profiles=PROFILES)
    if plan != expected:
        raise ValueError("Launch requires exactly the frozen default native plan for 3B base and staleness 2/4/6/8")
...
def run(args, package_sha256):
    ...
    with (scope / "run.lock").open("a") as lock:
        ...
        try:
            ...
            smoke_groups = [[] for _ in devices]
            for model_index, model_id in enumerate(MODEL_IDS):
                index = model_index % len(devices)
                smoke_groups[index].append(create_smoke(candidates[model_id], directory / "smoke-prepared", allocation_info, index, launch_id))
            smoke_lock = threading.Lock()

            def smoke_worker(index, device):
                for entry in smoke_groups[index]:
                    result = run_process(entry, directory / "smoke-results", device, directory / "logs" / f"gpu-{index}-smoke", children)
                    with smoke_lock:
                        receipt["smoke"].append(result)
                        write_json(receipt_path, receipt)
