...
class NativeMappingTests(unittest.TestCase):
    def setUp(self):
        self.visible=[
            ...
            {'local_ordinal':2,'uuid':'f25f0579-8800-4c90-8100-78e972a1b0d3','pci_bus_id':'0000:c1:00.0'},
        ]
        self.physical={
            0:{'minor':0,**self.visible[0]},
            1:{'minor':1,**self.visible[1]},
            2:{'minor':2,**self.visible[2]},
    ...
    def test_cuda_selectors_do_not_select_same_numbered_linux_minors(self):
        actual=bind_visible_devices('3,4,8','3,4,8',self.visible,self.physical)
        self.assertEqual([d['uuid'] for d in actual],[d['uuid'] for d in self.visible])
        self.assertEqual([d['minor'] for d in actual],[0,1,2])
    ...
    def test_noncontiguous_selectors_keep_visible_identities(self):
        actual=bind_visible_devices('2,7,8','2,7,8',self.visible,self.physical)
        self.assertEqual([d['slurm_visible_selector'] for d in actual],[2,7,8])
        self.assertEqual([d['uuid'] for d in actual],[d['uuid'] for d in self.visible])

    def test_duplicate_and_partial_devices_fail(self):
        visible=copy.deepcopy(self.visible);visible[1]['uuid']=visible[0]['uuid']
        with self.assertRaisesRegex(ValueError,'Duplicate visible'):bind_visible_devices('3,4,8','3,4,8',visible,self.physical)
    ...
    def test_selector_and_pci_mismatch_fail(self):
        with self.assertRaisesRegex(ValueError,'Unexpected Slurm selector'):bind_visible_devices('3,4,8','3,5,8',self.visible,self.physical)
        visible=copy.deepcopy(self.visible);visible[0]['pci_bus_id']='0000:ff:00.0'
        with self.assertRaisesRegex(ValueError,'PCI identity'):bind_visible_devices('3,4,8','3,4,8',visible,self.physical)

    def test_changed_assignment_fails_before_driver_query(self):
        env={'CUDA_VISIBLE_DEVICES':'0,1','RECOVERY_SLURM_CUDA_VISIBLE_DEVICES':'3,4,8','SLURM_JOB_GPUS':'3,4,8'}
        with patch.dict(os.environ,env,clear=True),patch('native_devices.socket.gethostname',return_value='deep-chungus-5'),patch('native_devices.visible_inventory') as query:
