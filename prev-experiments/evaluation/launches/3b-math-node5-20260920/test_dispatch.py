...
class DispatchTests(unittest.TestCase):
    ...
    def test_full_default_plan_is_required(self):
        plan = dispatch.make_plan(*dispatch.launch_catalogs(), selected_models=dispatch.MODEL_IDS, selected_profiles=dispatch.PROFILES)
    ...
    def test_smoke_failure_prevents_every_production_cell(self):
        ...
        allocation = {"job_id": "123", "worker_gpu_uuids": ["GPU-0", "GPU-1", "GPU-2"], "gpu_count": 3}
        calls = []
        ...
        def fake_run(entry, output_root, device, logs, children):
            with lock:
                ...
                if "smoke-results" in str(output_root):
                    smoke.add(device)
                    smoke_models.add(entry["cell"]["model_id"])
                else:
                    self.assertEqual(smoke, {"GPU-0", "GPU-1", "GPU-2"})
                    self.assertEqual(smoke_models, set(dispatch.MODEL_IDS))
