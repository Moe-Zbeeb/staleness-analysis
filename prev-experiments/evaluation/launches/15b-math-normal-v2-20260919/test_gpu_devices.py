...
class GpuMappingTests(unittest.TestCase):
    ...
    def test_slurm_ordinals_resolve_explicit_noncontiguous_device_minors(self):
        ...
        selected = gpu_devices.expected_devices([1, 3], files, inventory(), 2)
        ...
        single = gpu_devices.expected_devices([3], files, inventory(), 1)
        self.assertEqual([item["minor"] for item in single], [7])
    ...
    def test_duplicate_or_missing_allocated_uuid_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "not unique"):
            gpu_devices.expected_devices([1, 5], files, duplicate, 2)
        missing = inventory()
        missing.pop(5)
        with self.assertRaisesRegex(ValueError, "no NVIDIA identity"):
            gpu_devices.expected_devices([1, 5], files, missing, 2)

    def test_wrong_allocation_count_and_out_of_bounds_rejected(self):
        files = [f"/dev/nvidia{number}" for number in range(8)]
        for selected in ([1], [1, 2, 5], [1, 1], [1, 8]):
            with self.subTest(selected=selected), self.assertRaises(ValueError):
                gpu_devices.expected_devices(selected, files, inventory(), 2)
        for selected in ([], [1, 5], [8]):
            with self.subTest(single_allocation=selected), self.assertRaises(ValueError):
                gpu_devices.expected_devices(selected, files, inventory(), 1)
...
class HardwareTests(unittest.TestCase):
    def query(self, rows, processes="", node="deep-chungus-1", numbers=(1, 5)):
        devices = [inventory()[number] for number in numbers]
        with patch.object(gpu_devices.subprocess, "run", side_effect=[SimpleNamespace(stdout=rows), SimpleNamespace(stdout=processes)]) as run:
            ...
            for call in run.call_args_list:
                self.assertEqual(call.args[0][1], "--id=" + ",".join(identifier(number) for number in numbers))
    ...
    def test_single_allocated_node1_gpu_keeps_memory_gate_without_fallback(self):
        hardware, cohort = self.query(f"{identifier(6)}, NVIDIA A100-PCIE-40GB, 40960, 40445, 1\n", numbers=(6,))
        self.assertEqual([item["uuid"] for item in hardware], [identifier(6)])
        self.assertEqual(cohort, "a100-40gb")
        with self.assertRaisesRegex(ValueError, "insufficient free VRAM"):
            self.query(f"{identifier(7)}, NVIDIA A100-PCIE-40GB, 40960, 2284, 38161\n", numbers=(7,))
...
class AllocationTests(unittest.TestCase):
    ...
    def allocate(self, env=None, host="deep-chungus-11.csail.mit.edu"):
        def hardware(devices, node):
            return [{"uuid": item["uuid"]} for item in devices], gpu_devices.HARDWARE_CLASSES[node][0]
    ...
    def test_node1_single_gpu_uses_slurm_selection(self):
        environment = {**self.env, "SLURM_GPUS_ON_NODE": "1", "SLURM_JOB_GPUS": "6", "CUDA_VISIBLE_DEVICES": "7"}
        result = self.allocate(environment, host="deep-chungus-1")
        self.assertEqual(result["worker_gpu_uuids"], [identifier(6)])
        self.assertEqual(result["gpu_count"], 1)
        self.assertEqual(result["slurm_gpu_ids"], [6])
        self.assertEqual(result["hardware_cohort"], "a100-40gb")
        self.assertEqual(result["original_cuda_visible_devices"], "7")
        another = self.allocate({**environment, "SLURM_JOB_GPUS": "3"}, host="deep-chungus-1")
        self.assertEqual(another["worker_gpu_uuids"], [identifier(3)])

    def test_node_specific_cardinality_is_required(self):
        with self.assertRaisesRegex(ValueError, "exactly 1 allocated GPUs"):
            self.allocate(host="deep-chungus-1")
        environment = {**self.env, "SLURM_GPUS_ON_NODE": "1", "SLURM_JOB_GPUS": "6"}
        with self.assertRaisesRegex(ValueError, "exactly 2 allocated GPUs"):
            self.allocate(environment, host="deep-chungus-11")
        with self.assertRaisesRegex(ValueError, "exactly 1 distinct Slurm"):
            self.allocate({**environment, "SLURM_JOB_GPUS": "6,7"}, host="deep-chungus-1")
