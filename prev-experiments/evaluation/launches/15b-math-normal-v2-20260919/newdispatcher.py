...
def partition_entries(entries, shards):
    ...
    for key, cost, values in sorted(weighted, key=lambda item: (-item[1], item[0])):
        shard = min(range(shards), key=lambda index: ((loads[index] + cost) / GPU_COUNTS[index], loads[index] / GPU_COUNTS[index], index))
        loads[shard] += cost
    ...
    return assigned, {"schema_version": 1, "shards": shards, "shard_nodes": {str(key): value for key, value in SHARD_NODES.items()}, "gpu_counts": {str(key): value for key, value in GPU_COUNTS.items()}, "grouping": ["profile", "benchmark"], "groups": decisions, "maximum_output_tokens_by_shard": loads, "maximum_output_tokens_per_gpu": [value / GPU_COUNTS[index] for index, value in enumerate(loads)], "cell_counts": [len(items) for items in assigned]}
...
def run(args, source, source_sha256, package_sha256):
    ...
    expected_gpus = GPU_COUNTS[args.shard]
    if allocation_info["gpu_count"] != expected_gpus or len(set(allocation_info["worker_gpu_uuids"])) != expected_gpus:
        raise ValueError(f"Shard {args.shard} requires exactly {expected_gpus} distinct allocated GPUs")
