...
GPU_COUNTS = {"deep-chungus-1": 1, "deep-chungus-11": 2}
ALLOWED_NODES = set(GPU_COUNTS)
...
def expected_devices(allocated, configured_files, inventory, expected_count):
    require(expected_count in (1, 2), "Unsupported authorized GPU count")
    require(len(allocated) == expected_count and len(set(allocated)) == expected_count, f"Expected exactly {expected_count} distinct Slurm GPU IDs")
    result = []
    ...
    require(len({entry["uuid"] for entry in result}) == expected_count, "Allocated NVIDIA GPU UUIDs are not unique")
    return result
...
def allocation(args):
    ...
    expected_count = GPU_COUNTS[node]
    require(os.environ.get("SLURM_GPUS_ON_NODE", "") == str(expected_count), f"GPU mapping requires exactly {expected_count} allocated GPUs on {node}")
    ...
    devices = expected_devices(allocated, configured, proc_inventory(), expected_count)
