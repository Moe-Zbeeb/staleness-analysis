...
def inspect_allocation():
    ...
    require(job.isdigit() and os.environ.get('SLURM_GPUS_ON_NODE') == '9', 'Requires nine GPUs in a Slurm job')
    ...
    require(len(inventory) == 9 and {item['uuid'] for item in inventory} == set(selected), 'Incomplete or duplicate allocated GPU inventory')
    require(len({(item['name'], item['total_memory_mib']) for item in inventory}) == 1, 'All nine workers must have identical GPU hardware')
    ...
    require(memory['MemAvailable'] >= 216 * 1024**3, 'Less than 216 GiB host RAM available for nine workers')
    return {'job_id': job, 'host': socket.gethostname(), 'gpu_count': 9, 'worker_gpu_uuids': selected, 'inventory': inventory, 'allocated_devices': devices, 'original_cuda_visible_devices': visible, 'slurm_job_gpus': os.environ['SLURM_JOB_GPUS'], 'cuda_device_order': os.environ['CUDA_DEVICE_ORDER'], 'gres_config_sha256': sha, 'memory_bytes': memory, 'preexisting_compute_processes': processes, 'assignment_basis': 'Original Slurm CUDA_VISIBLE_DEVICES, resolved through CUDA with PCI_BUS_ID ordering'}
