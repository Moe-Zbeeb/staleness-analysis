...
def visible_inventory():
    ...
    require(count.value == GPU_COUNT, f"Slurm must expose exactly nine CUDA devices, got {count.value}")
    result = []
...
def bind_visible_devices(cvd, allocated, visible, physical):
    ...
    require(len(selectors) == GPU_COUNT, "Expected nine original Slurm visible selectors")
