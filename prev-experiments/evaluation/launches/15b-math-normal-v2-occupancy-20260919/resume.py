...
def verify_package(root=PACKAGE):
    ...
    if manifest.get("schema_version") != 1 or not {"resume.py", "evaluate_job.sh"} <= manifest.get("files", {}).keys():
...
def gated_allocation(args, original_allocation, package_sha256, source):
    ...
    provenance = {
        ...
        "policy": "Reject every pre-existing compute process on each assigned UUID",
    }
    directory = args.results_root / ".occupancy-recovery" / f"{info['job_id']}-{info['restart_count']}"
...
def main():
    ...
    gpu_devices.allocation = lambda options: gated_allocation(options, original_allocation, package_sha256, source)
    try:
