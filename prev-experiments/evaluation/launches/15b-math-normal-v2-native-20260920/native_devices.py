...
def visible_inventory():
    ...
    require(count.value == GPU_COUNT, f"Slurm must expose exactly two CUDA devices, got {count.value}")
    result = []
...
def bind_visible_devices(cvd, allocated, visible, physical):
    ...
    require(len(selectors) == GPU_COUNT, "Expected two original Slurm visible selectors")
...
def inspect_assignment():
    require(socket.gethostname().split(".")[0] == "deep-chungus-11", "Native-selector recovery is validated for deep-chungus-11 only")
