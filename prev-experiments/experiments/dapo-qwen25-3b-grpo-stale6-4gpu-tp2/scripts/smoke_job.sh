...
uv run --no-sync torchrun --standalone --nproc-per-node=4 "$EXPERIMENT_ROOT/scripts/health_probe.py"
...
SMOKE_NAME="qwen25-3b-grpo-seed42-stale6-4gpu-tp2-smoke-$SLURM_JOB_ID-attempt-${SLURM_RESTART_COUNT:-0}"
SMOKE_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-stale6-4gpu-tp2/$SMOKE_NAME"
