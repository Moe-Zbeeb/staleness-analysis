...
def verify_reference_data(reference: Path, generated: dict[str, list[dict]]) -> dict:
    ...
    for name, values in generated.items():
        ...
        if semantic(values) != semantic(source_rows):
            raise ValueError(f"The 14B rows or their order differ from the 7B experiment: {name}")
...
def main() -> None:
    ...
    reference_tokenizer = AutoTokenizer.from_pretrained(args.reference_data.parent / "tokenizer", local_files_only=True)
    tokenizer.chat_template = reference_tokenizer.chat_template + "{% if add_generation_prompt %}{{ '<think>\\n\\n</think>\\n\\n' }}{% endif %}"
