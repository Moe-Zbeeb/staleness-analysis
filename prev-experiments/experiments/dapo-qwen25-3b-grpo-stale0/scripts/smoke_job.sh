...
uv run --no-sync torchrun --standalone --nproc-per-node=7 "$EXPERIMENT_ROOT/scripts/health_probe.py"
...
SMOKE_NAME="qwen25-3b-grpo-seed42-stale0-smoke-$SLURM_JOB_ID-attempt-${SLURM_RESTART_COUNT:-0}"
SMOKE_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-stale0/$SMOKE_NAME"
...
uv run --no-sync python "$EXPERIMENT_ROOT/scripts/staleness_guard.py" smoke --experiment-root "$EXPERIMENT_ROOT" --run-dir "$SMOKE_DIR" --steps 5
