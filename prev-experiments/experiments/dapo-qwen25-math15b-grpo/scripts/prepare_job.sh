...
MODEL=/mnt/nfs/home/mohamadzbib/projects/models/Qwen2.5-Math-1.5B-4a83ca6e
...
uv run --no-sync python "$EXPERIMENT_ROOT/scripts/prepare_model.py" --spec "$EXPERIMENT_ROOT/experiment.json"
...
uv run --no-sync python "$EXPERIMENT_ROOT/scripts/prepare_data.py" --output-dir "$EXPERIMENT_ROOT/data" --model "$MODEL" --tokenizer-output "$EXPERIMENT_ROOT/tokenizer" --intellect-root "$INTELLECT_ROOT" --exclusions "$EXPERIMENT_ROOT/config/data_exclusions.json" --experiment-root "$EXPERIMENT_ROOT" --observer-archive "$OBSERVER_ARCHIVE" --reference-data "$RL_INFRA/experiments/dapo-qwen25-math7b-grpo/data" --seed 42
