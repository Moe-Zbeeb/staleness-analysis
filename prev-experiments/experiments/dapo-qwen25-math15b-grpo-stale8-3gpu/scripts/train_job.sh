set -euo pipefail
EXPERIMENT_ROOT=/mnt/nfs/home/mohamadzbib/projects/rl-infra/experiments/dapo-qwen25-math15b-grpo-stale8-3gpu
RL_INFRA=/mnt/nfs/home/mohamadzbib/projects/rl-infra
RUN_NAME=qwen25-math15b-grpo-seed42-stale8-3gpu
ATTEMPT_ID="job-$SLURM_JOB_ID-restart-${SLURM_RESTART_COUNT:-0}"
RUN_DIR="$RL_INFRA/outputs/dapo-qwen25-math15b-grpo-stale8-3gpu/$RUN_NAME"
OBSERVER_DIR="$RL_INFRA/outputs/dapo-qwen25-math15b-grpo-stale8-3gpu-observer/$RUN_NAME"
RECOVERY_DIR="$RL_INFRA/outputs/dapo-qwen25-math15b-grpo-stale8-3gpu-recovery/$RUN_NAME"
...
MODEL_LINK=/mnt/xfs/home/mohamadzbib/projects/models/qwen25-math15b-grpo-seed42-stale8-3gpu
...
verify_credentials() (
    ...
    export OPIK_PROJECT_NAME=qwen25-math15b-grpo-stale8-3gpu
)
...
if test "$TRAINING_FINISHED" = false; then
    ...
    run_guarded uv run --no-sync torchrun --standalone --nproc-per-node=3 "$EXPERIMENT_ROOT/scripts/health_probe.py"
