...
RUN_NAME=qwen25-3b-grpo-seed42
ATTEMPT_ID="job-$SLURM_JOB_ID-restart-${SLURM_RESTART_COUNT:-0}"
RUN_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo/$RUN_NAME"
OBSERVER_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-observer/$RUN_NAME"
RECOVERY_DIR="$RL_INFRA/outputs/dapo-qwen25-3b-grpo-recovery/$RUN_NAME"
...
MODEL_LINK=/mnt/xfs/home/mohamadzbib/projects/models/qwen25-3b-grpo-seed42
...
verify_credentials() (
    ...
    export OPIK_PROJECT_NAME=qwen25-3b-grpo
)
...
IFS=, read -r -a GPU_IDS <<< "$PRIME_VISIBLE_DEVICES"
test "${#GPU_IDS[@]}" -eq 7
PRIME_VISIBLE_DEVICES="${GPU_IDS[4]},${GPU_IDS[5]},${GPU_IDS[6]},${GPU_IDS[0]},${GPU_IDS[1]},${GPU_IDS[2]},${GPU_IDS[3]}"
