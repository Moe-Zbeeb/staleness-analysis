...
def main() -> None:
    ...
    if torch.cuda.device_count() != 8 or dist.get_world_size() != 8:
        raise ValueError("The allocation must expose exactly eight GPUs")
    device = torch.cuda.get_device_properties(local_rank)
    if "H100" not in device.name or device.total_memory < 38 * 1024**3:
        raise ValueError("Expected H100 with at least 38 GiB per allocated device")
