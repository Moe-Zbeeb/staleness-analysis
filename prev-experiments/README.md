...
| [1.5B cap 6](experiments/dapo-qwen25-math15b-grpo-stale6-3gpu/README.md) | Completed 1,000 updates; final checkpoint and all nine evaluations verified | 2 + 1 |
...
| [3B cap 4](experiments/dapo-qwen25-3b-grpo-stale4-5gpu/README.md) | Five-GPU health check passed; smoke started on deep-chungus-7 at normal priority | 3 + 2 |
| [3B cap 6 retry](experiments/dapo-qwen25-3b-grpo-stale6-4gpu-tp2/README.md) | Job 2141859 queued on deep-chungus-9 at normal priority; source and resolved configuration validated; 25-step smoke gates production. Failed job 2141854 is preserved. | 2 + 2 (inference TP2/DP1) |
| [3B cap 8](experiments/dapo-qwen25-3b-grpo-stale8-4gpu/README.md) | Job 2141858 pending on deep-chungus-9 at normal priority (Slurm: Resources); replaces never-started job 2141820; startup checks pending | 2 + 2 (inference TP2/DP1) |
...
The four-GPU 3B cap-6 attempt (job 2141854) passed GPU health but failed during smoke inference startup: the vLLM engines timed out waiting for API frontend processes. Production did not start. Its logs and frozen configuration are preserved.

The active 3B cap-8 arm requests four A100s, 32 CPUs and 160 GiB host memory on deep-chungus-9. Job 2141858 replaces the never-started six-GPU job 2141820. It assigns two GPUs to training and uses a single inference replica split across two GPUs (TP2/DP1), with one API process to avoid the multiprocess frontend startup path that failed for cap 6. Its deployed source and resolved configuration passed validation. Scheduling is grad-students / low-priority / normal under the user's scheduling override, with unlimited Slurm wall time and same-arm checkpoint recovery. A separate 25-update smoke gates fresh 1,000-update production. The declared GPU topology, inference parallelism, serving-process count, run identity and network ports differ from the baseline and are recorded alongside the staleness treatment.
